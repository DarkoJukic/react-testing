...
//   }
  ...
  renderTags() {
    if (this.state.tags.length === 0) return <p>No Tags</p>;
      ...
      return this.state.tags.map(tag => <li   onClick={() => this.handleTagClick(tag)}  key={tag}>{tag}</li>);
    }
  }
  ...
  console.log(this.props)
    return (
      <React.Fragment>
        ...
        <button onClick={this.handleIncrement2} className="btn btn-secondary btn-sm">Increment</button>
        <button onClick={() => this.props.onIncrement(this.props.counter)} className="btn btn-secondary btn-sm">Increment Controlled</button>
